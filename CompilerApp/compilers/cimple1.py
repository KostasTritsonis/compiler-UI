...
class Entity():
    def __init__(self):
        self.name = ''
        self.type = '' # VAR or SUB or PAR or TEMP
...
def add_parameters():
    ...
    for a in topScope.enScope.entity[-1].subprogram.argument:
        e = Entity()
        e.name = a.name
        e.type = 'PAR'
        e.parameter.mode = a.parMode
        new_entity(e)
...
def write_Symbol_table():
    ...
    scope=topScope
    while scope != None:
        cp.write("SCOPE: "+"name:"+scope.name+"\tnestingLevel:"+str(scope.nestingLevel)+"\n")
        cp.write("\tENTITIES:"+"\n")
        for e in scope.entity:
            if(e.type == 'VAR' or e.type == 'TEMP'):
                cp.write("\tENTITY: "+" name:"+e.name+"\t type:"+e.type+"\n")
            elif(e.type == 'SUB'):
                cp.write("\tENTITY: "+" name:"+e.name+"\t type:"+e.type+"\t subprogram-type:"+e.subprogram.type+"\n")
                cp.write("\t\tARGUMENTS:"+"\n")
                for a in e.subprogram.argument:
                    cp.write("\t\tARGUMENT: "+" name:"+a.name+"\t type:"+a.type+"\t argMode:"+a.parMode+"\n")
            elif(e.type == 'PAR'):
                cp.write("\tENTITY: "+" name:"+e.name+"\t type:"+e.type+"\t parmode:"+e.parameter.mode+"\n")
        cp.write("\n")
        scope = scope.enScope    
